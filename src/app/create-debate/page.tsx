...
export default function CreateDebatePage() {
  ...
  const router = useRouter();
  const [createDebate, { isLoading: isSubmitting }] = useCreateDebateMutation();
  const {
    isUploading,
    uploadedUrl,
    error: uploadError,
  ...
  const onSubmit = async (data: DebateFormData) => {
    try {
      ...
      const debateData: DebateCreateInput = {
        title: data.title,
        description: data.description,
        category: data.category,
        duration: Number.parseInt(data.duration) * 60 * 60 * 1000, // Convert hours to milliseconds
        tags: tags.length > 0 ? tags : undefined,
        image: imageUrl,
      };

      const response = await createDebate(debateData).unwrap();
      toast.success(
        "Your debate has been successfully created and is now live."
      );
      router.push("/"); // Assuming API returns debate ID
    } catch (error) {
