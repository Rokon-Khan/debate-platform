...
// The API response should match this interface
interface LeaderboardUser {
  id: string;
  username: string;
  avatar?: string;
...
// For stats summary, assume response from scoreboard API includes these keys
...
export default function ScoreboardPage() {
  ...
  // Use RTK Query to fetch scoreboard data
  // Assume API returns { leaderboard: LeaderboardUser[], stats: ScoreboardStats }
  const { data, isLoading } = useGetScoreboardQuery({ filter: timeFilter });

  console.log("Scoreboard data:", data);

  const leaderboard: LeaderboardUser[] = data?.leaderboard ?? [];
  const stats: ScoreboardStats = data?.stats ?? {};
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <div className="font-semibold">{user.username}</div>
                          <div className="text-sm text-muted-foreground">
                            {user.debatesParticipated} debates • {user.winRate}%
                            win rate
                          </div>
                        </div>
                      </div>
                ...
                <div>
                  <div className="text-2xl font-bold">
                    {isLoading ? (
                      ...
                      stats.activeDebaters?.toLocaleString() ?? 0
                    )}
                  </div>
                  <div className="text-sm text-muted-foreground">
                    Active Debaters
                ...
                <div>
                  <div className="text-2xl font-bold">
                    {isLoading ? (
                      ...
                      stats.debatesThisWeek?.toLocaleString() ?? 0
                    )}
                  </div>
                  <div className="text-sm text-muted-foreground">
                    Debates This Week
                ...
                <div>
                  <div className="text-2xl font-bold">
                    {isLoading ? (
                      ...
                      stats.totalArguments?.toLocaleString() ?? 0
                    )}
                  </div>
                  <div className="text-sm text-muted-foreground">
                    Total Arguments
